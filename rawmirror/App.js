...
export default function App() {
  ...
  if (authState === 'loading') {
    return <View style={[styles.loading]}><ActivityIndicator color={T.accent} size="large" /></View>;
  }
  if (authState === 'setup') return <PINScreen isSetup onSuccess={() => setAuthState('unlocked')} />;
  if (authState === 'pin') return <PINScreen onSuccess={() => setAuthState('unlocked')} />;

  // ─── OVERLAY SCREENS (full screen, no bottom nav) ──────────────────────
  if (overlayScreen === 'result' && selectedEntry) {
    return <EntryResultScreen entry={selectedEntry} onBack={closeOverlay} onToggleMoment={toggleMoment} />;
  }
    ...
    return (
      <View style={{ flex: 1 }}>
        ...
        <View style={styles.backOverlay}>
          <BottomNav active="settings" onPress={(tab) => { setOverlayScreen(null); setActiveTab(tab); }} />
        </View>
      </View>
    );
  }
  ...
  return (
    <View style={{ flex: 1, backgroundColor: T.bg }}>
      <View style={{ flex: 1 }}>
        {activeTab === 'home' && (
          <HomeScreen entries={entries} onNewEntry={() => setActiveTab('journal')} onViewEntry={openEntry} />
        )}
        {activeTab === 'journal' && (
          <JournalScreen entries={entries} settings={settings} onSave={saveEntry} existingDraft={todayEntry?.isDraft ? todayEntry : null} />
        )}
        {activeTab === 'timeline' && (
        ...
        {activeTab === 'settings' && (
          <SettingsScreen settings={settings} onSave={saveSettings} entries={entries} onNavigate={setOverlayScreen} />
        )}
      </View>
